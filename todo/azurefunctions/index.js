...
document.getElementById("focus").scrollIntoView();
    ...
    var updateTodos = function(updatetodo) {
      ...
      req.open('POST','https://sfsqltodo.azurewebsites.net/api/todo', false);
...
module.exports = async function (context, req) {
    const ipa = req.headers['x-forwarded-for']
    const newipa = ipa.substr(0, ipa.lastIndexOf(":"));
    const ip = newipa.replaceAll('.', '').replaceAll(':', '');
context.log(ipa);
context.log(ip);
    ...
    if (req.method    === 'POST') {
      ...
      async function post(url, data) {
                    ...
                    return new Promise((resolve, reject) => {
                        const postreq = https.request(url, options, (postres) => {
                        if (postres.statusCode < 200 || postres.statusCode > 299) {
                            return reject(new Error(`HTTP status code ${postres.statusCode}`))
                        }

                        const body = []
                        postres.on('data', (chunk) => body.push(chunk))
                        postres.on('end', () => {
                        ...
                        postreq.end()
                           ...
                           const json = await post(process.env.endpoint,putData)
        ...
        async function post(url, data) {
            ...
            const postdata = JSON.parse(await post(process.env.endpoint,QData))
              if(postdata[0]["data"] == undefined){
                     const todos = defaultData.todos
                     const resultsbody = html(JSON.stringify(todos || []).replace(/</g, "\\u003c"))
                            context.res = { body: resultsbody,
                             headers: { 'content-type': 'text/html',
                                        'content-length': resultsbody.length },
                                };
                }else{
                    const todos = postdata[0]["data"];	
                    const resultsbody = html(JSON.stringify(todos || []).replace(/</g, "\\u003c"))
                         context.res = { body: resultsbody,
                             headers: { 'content-type': 'text/html',
                                        'content-length': resultsbody.length },
