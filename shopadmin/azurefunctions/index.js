...
const sfsqlApiUrl = process.env.endpoint;
...
const init = {
    ...
    context.log(sfsqlReqPayload);
    const sfsql_response = await fetch(sfsqlApiUrl, init);
    //const sfsql_response = await axios.post(config,sfsqlReqPayload);
    //const json = await sfsql_response.json();
    const json = await sfsql_response.json()
    const body = html(JSON.stringify(json));
*/
...
const json = await post(process.env.endpoint,sfsqlReqPayload)

//const body = html(JSON.stringify(json));
const resultsbody = html(json);
    context.res = {
                    body: resultsbody,
